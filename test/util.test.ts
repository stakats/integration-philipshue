...
test("buildSceneOptionsForGroup returns just the placeholder for an empty group", (t) => {
  const result = buildSceneOptionsForGroup([]);
  t.deepEqual(result.options, [SCENE_NONE_OPTION]);
  t.is(result.optionToId.size, 0);
  t.is(result.idToOption.size, 0);
});

test("buildSceneOptionsForGroup places the placeholder first and sorts scenes alphabetically", (t) => {
  const scenes = [makeScene("s1", "Bright"), makeScene("s2", "Cozy"), makeScene("s3", "Ambient")];
  const result = buildSceneOptionsForGroup(scenes);
  t.deepEqual(result.options, [SCENE_NONE_OPTION, "Ambient", "Bright", "Cozy"]);
...
test("buildSceneOptionsForGroup suffixes duplicate scene names with (2), (3)", (t) => {
  ...
  const result = buildSceneOptionsForGroup(scenes);
  t.deepEqual(result.options, [SCENE_NONE_OPTION, "Movie Night", "Movie Night (2)", "Movie Night (3)"]);
